...
Here we provide a guide to run HGC in `Seurat` pipeline and utilize the 
...
Some scRNA-seq processing pipelines include 
[`igraph`](https://igraph.org/r/) package 
to run the graph-related calculation, like 
[`scran`](https://bioconductor.org/packages/scran), 
[`OSCA`](http://bioconductor.org/books/release/OSCA/), 
etc.

The igraph package has the specific data structure to 
save graphs, and the pipelines utilize the structure 
to save the graph built on scRNA-seq datasets. Here 
we show a guild for utilizing `HGC` to cluster `igraph` 
object in `scran` pipeline.
...
cluster.tree <- cluster_HGC(graph = g)
